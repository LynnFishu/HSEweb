...
const AdminMap = () => {
  ...
  // Filter orders that have valid site locations
  const ordersWithLocations = workOrders.filter(order => 
    order.siteLocation && 
  ...
  const mapCenter = ordersWithLocations.length > 0 
    ? {
        lat: ordersWithLocations[0].siteLocation.lat,
        lng: ordersWithLocations[0].siteLocation.lng
      }
    : center;
      ...
      {selectedOrder && (
        ...
        >
          <Box sx={{ p: 1, maxWidth: 200 }}>
            ...
            <Typography variant="body2" sx={{ color: '#677294', mb: 1 }}>
              Requested by: {selectedOrder.requestedBy}
            </Typography>
            <Chip
